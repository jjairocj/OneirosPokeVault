    ...
    <div className="min-h-screen bg-gray-900 text-white p-6 max-w-2xl mx-auto">
      {bannerImage && (
        <div className="h-32 rounded-xl overflow-hidden mb-6">
          <img src={bannerImage} alt="Banner" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
        </div>
      )}

      <h1 className="text-2xl font-bold mb-1">{profile?.displayName || user?.email}</h1>
      <p className="text-xs text-gray-500 mb-6">{user?.plan?.toUpperCase()} plan · {user?.email}</p>
      ...
      <form onSubmit={handleSave} className="bg-gray-800 rounded-xl p-5 flex flex-col gap-4 mb-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            type="submit"
            disabled={saving}
            className="ml-auto bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm px-4 py-2 rounded"
          >
